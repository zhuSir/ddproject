...
public class ExcelUtil {
    ...
    // 构造方法，传入要导出的数据
    public ExcelUtil(String title, List<String> rowName, List<Object[]> dataList, String fileName, HttpServletResponse response) {
     ...
     * 导出数据
     */
    public double export() throws Exception {
        double xlsLength = 0;
        try {
            ...
            if (workbook != null) {
                try {
                    String fileName = URLEncoder.encode(this.fileName, "UTF-8") + ".xls";
                    ...
                    bos.close();
                    out.close();
                    xlsLength = bos.toByteArray().length / 1024;
                } catch (IOException e) {
                    e.printStackTrace();
                }
            }
